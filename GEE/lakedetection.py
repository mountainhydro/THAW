...
today = datetime.datetime(2025,7,12); # Purepu
today
...
s1_asc = apply_radar_mask_to_collection(s1_asc_raw, elev)

s1_desc_raw = s1 \
    .filterDate(start, today) \
    .filter(ee.Filter.eq('orbitProperties_pass', 'DESCENDING')) \
    .sort('system:time_start', False)
s1_desc =  apply_radar_mask_to_collection(s1_desc_raw, elev)
...
vis_params = {'min': -30, 'max': 0, 'palette': ['black', 'white']}

Map2 = geemap.Map(basemap='SATELLITE', ee_initialize=False)
Map2.add_basemap('OpenStreetMap')
Map2.add_basemap('Esri.WorldTopoMap')
Map2.add_basemap('Google.Terrain')
#Map2.addLayer(s1.select('VV'), vis_params, 'Raw Mean VV (no mask)')
Map2.addLayer(s1_asc_raw.select('VV'), vis_params, 'Raw Mean VV (no mask)')
Map2.addLayer(mean_img.select('VV'), vis_params, 'Mean VV (no mask)')
